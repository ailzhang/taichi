...
namespace aot {
void Dispatch::run(taichi::lang::RuntimeContext *host_ctx,
  ...
  for (int i = 0; i < symbolic_args_.size(); ++i) {
    ...
    if (ival.tag == IValue::Tag::DEVALLOC) {
      host_ctx->set_arg_devalloc(
          i, *(reinterpret_cast<DeviceAllocation *>(ival.val)), ival.shape_);
    } else {
...
void Sequential::run(taichi::lang::RuntimeContext *host_ctx,
                     std::unordered_map<std::string, IValue> &args) {
