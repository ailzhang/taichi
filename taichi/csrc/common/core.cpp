...
#include "taichi/csrc/common/version.h"
#include "taichi/csrc/common/commit_hash.h"
...
std::string get_version_string() {
  //return fmt::format("{}.{}.{}", get_version_major(), get_version_minor(),
                     //get_version_patch());
  return TI_VERSION_STRING;
}

int get_version_major() {
  auto version = split_string(TI_VERSION_STRING, ".");
  return std::atoi(version[0].c_str());
}

int get_version_minor() {
  auto version = split_string(TI_VERSION_STRING, ".");
  return std::atoi(version[1].c_str());
}

int get_version_patch() {
  auto version = split_string(TI_VERSION_STRING, ".");
  return std::atoi(version[2].c_str());
}
