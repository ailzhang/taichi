 ...
 public:
  ...
  void visit(ArgLoadStmt *stmt) override {
    ...
    if (stmt->is_ptr) {
      ...
      const auto dt = vk_dtype_enum_to_dt(arg_attribs.dtype);
 ...
 private:
    ...
    for (auto &arg : ctx_attribs_->args()) {
      if (arg.is_array &&
        ...
        struct_components_.emplace_back(
            ir_->get_primitive_type(vk_dtype_enum_to_dt(arg.dtype)),
  ...
  void compile_ret_struct() {
    ...
    for (auto &ret : ctx_attribs_->rets()) {
      if (auto tensor_type =
              vk_dtype_enum_to_dt(ret.dtype)->cast<TensorType>()) {
        struct_components_.emplace_back(
            ir_->get_array_type(
        ...
        struct_components_.emplace_back(
            ir_->get_array_type(
                ir_->get_primitive_type(vk_dtype_enum_to_dt(ret.dtype)), 1),
            "ret" + std::to_string(ret.index), ret.offset_in_mem);
      }
    }
