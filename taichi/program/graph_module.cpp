...
void Dispatch::run(std::unordered_map<std::string, IValue> &args) {
  ...
  for (int i = 0; i < symbolic_args_.size(); ++i) {
    ...
    if (ival.tag == IValue::Tag::NDARRAY) {
      ...
      launch_ctx.set_arg_external_array_with_shape(i, ival.val, ival.size_,
                                                   ival.shape_);
    } else {
...
std::vector<aot::SymbolicDispatch> Dispatch::serialize(
    AotModuleBuilder *aot_builder) const {
  if (!serialized_) {
...
std::vector<aot::SymbolicDispatch> Sequential::serialize(
    AotModuleBuilder *aot_builder) const {
...
void Graph::serialize(AotModuleBuilder *aot_builder) const {
  aot_builder->add_graph(name_,
                         aot::DispatchSeq{this->seq()->serialize(aot_builder)});
}
}  // namespace lang
}  // namespace taichi
